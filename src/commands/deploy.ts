...
import { ComponentSlugUtils, ComponentVersionSlugUtils, DependencyNode, ServiceVersionSlugUtils, Slugs } from '../dependency-manager/src';
...
export default class Deploy extends DeployCommand {
  ...
  protected async runRemote(): Promise<void> {
    ...
    for (const service_node of service_nodes) {
      console.log(JSON.stringify(service_node));
