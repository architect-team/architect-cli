...
namespace ProtocExecutor {
  ...
  export const clear = async (target: ServiceDependency, start: Date) => {
    ...
    for (const dir of await fs.readdir(stub_directory)) {
      const lstat = await fs.lstat(path.join(stub_directory, dir));
      if (!lstat.isDirectory()) { continue; }
      try {
        const stat = await fs.stat(path.join(stub_directory, dir, 'checksum'));
        if (stat.mtime < start) {
          await fs.remove(path.join(stub_directory, dir));
        }
      } catch {
        const sub_dirs = [];
        const to_remove = [];
        for (const sub_dir of await fs.readdir(path.join(stub_directory, dir))) {
          const sub_dir_path = path.join(stub_directory, dir, sub_dir);
