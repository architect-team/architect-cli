...
class LocalServiceDependency extends ServiceDependency {
  ...
  tag(tag?: string) {
    if (tag) {
      return `${this.app_config.registry_host}/${this.config.name}:${tag}`;
    } else {
      return `${this.app_config.registry_host}/${this.config.full_name}`;
    }
  }
...
class DockerServiceDependency extends ServiceDependency {
  ...
  async _load() {
    const default_registry_host = this.app_config.registry_host;
