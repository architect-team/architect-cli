...
| -------------------- | ---------- | -------------- | -------------- |
 ...
 | `path` | string \| [Expression](/docs/reference/contexts) | Path for the http check executable. Path should be absolute (e.g. /health). This field is disjunctive with `command` (only `path` or `command` can be set). | Must match: <a target="_blank" href="https://regexr.com/?expression=%5E%5C%2F.*%24">Regex</a> |
 ...
 | `port`* | number \| [Expression](/docs/reference/contexts) | Port that the http check will run against |  |
