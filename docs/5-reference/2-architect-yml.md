...
| -------------------- | ---------- | -------------- | -------------- |
 ...
 | `path` | string \| [Expression](/docs/reference/contexts) | Path for the http check executable. Path should be absolute (e.g. /health). This field is disjunctive with `command` (only `path` or `command` can be set). |  |
