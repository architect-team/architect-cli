...
describe('register', function () {
  ...
  mockArchitectAuth
    ...
    .it('register multiple apps at the same time with a shared tagged version', ctx => {
      ...
      expect(ctx.stderr).to.contain('Registering component examples/gcp-pubsub:1.0.0 with Architect Cloud...... done\n');
      expect(ctx.stdout).to.contain('Successfully registered component');
    });
  ...
  mockArchitectAuth
    ...
    .stdout({ print })
    .stderr({ print })
    .command(['register', 'examples/react-app/architect.yml', '-t', '1.0.0', 'examples/gcp-pubsub/pubsub/architect.yml', '--arg', 'NODE_ENV=dev', '-a', 'examples' ])
