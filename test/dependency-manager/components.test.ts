...
describe('components spec v1', function () {
  describe('standard components', function () {
    ...
    it('simple remote component', async () => {
      ...
      const api_ref = resourceRefToNodeRef('architect/cloud.services.api:v1');
    ...
    it('local component with local dependency', async () => {
      ...
      expect(api_node.config.ref).to.eq('cloud.services.api:latest');
      ...
      expect(worker_node.config.ref).to.eq('concourse/ci.services.worker:6.2');
    });
    ...
    it('component B:v2 and component A with dependency B:v1', async () => {
      ...
      const graph = await manager.getGraph([
        ...
        await manager.loadComponentSpec('examples/component-b:v2')
      ], {
        '*': { test_required: 'foo1' },
        'examples/component-b:v1': {
          test_required: 'foo3'
      ...
      const api_ref = resourceRefToNodeRef('examples/component-b.services.api:v1');
    ...
    it('component with deep dependencies', async () => {
      ...
      const a_ref = resourceRefToNodeRef('examples/component-a.services.app:latest');
      const b_ref = resourceRefToNodeRef('examples/component-b.services.api:latest');
      const c_ref = resourceRefToNodeRef('examples/component-c.services.api:latest');
