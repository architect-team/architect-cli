...
describe('expression-interpolation', function () {
  ...
  it('loadParameters-with-expressions', async () => {
    const frontend_config = {
      ...
      parameters: {
        APP_PORT: 8080,
        DB_USER: "${ dependencies['db'].parameters.DB_USER }",
    ...
    const backend_datastore_node = graph.nodes[2] as ServiceNode;
    console.log('graph;', JSON.stringify(graph));
    console.log('frontend_node;', JSON.stringify(frontend_node));
    console.log('backend_node;', JSON.stringify(backend_node));
    console.log('backend_datastore_node;', JSON.stringify(backend_datastore_node));
    expect(Object.keys(frontend_node.parameters)).members(['APP_PORT', 'DB_USER', 'DEP_DB_USER', 'lower_dep_ADMIN_PORT']);
    expect(Object.keys(frontend_node.node_config.getEnvironmentVariables())).members(['APP_PORT', 'DB_USER', 'DEP_DB_USER', 'lower_dep_ADMIN_PORT', ...interface_env_variables]);
